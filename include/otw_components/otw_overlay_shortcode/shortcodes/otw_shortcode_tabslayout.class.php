<?php
class OTW_Overlay_Shortcode_TabsLayout extends OTW_Overlay_Shortcodes{
	...
	public function __construct(){
		...
		$this->shortcode_name = 'otw_shortcode_tabslayout';
	}
	/**
	...
	public function register_external_libs(){
		...
		$this->add_external_lib( 'js', 'otw-shortcode_live_preview', $this->component_url.'js/otw_shortcode_live_preview.js', 'live_preview', 200, array( 'jquery' ) );
	}
	/**
	 * apply settings
	 */
	...
	public function display_shortcode( $attributes, $content ){
		...
		if( $tabs = $this->format_attribute( '', 'tabs', $attributes, false, '' ) ){
			...
			for( $cT = 1; $cT <= $tabs; $cT++ ){
				...
				if( $tab_icon_type = $this->format_attribute( '', 'tab_'.$cT.'_icon_type', $attributes, false, '' ) ){
					$html .= '<i class="'.esc_attr( $tab_icon_type ).'"> </i>';
				}
				if( $tab_icon_url = $this->format_attribute( '', 'tab_'.$cT.'_icon_url', $attributes, false, '' ) ){
					$html .= '<img src="'.esc_attr( $tab_icon_url ).'" title="icon" alt="icon" />';
				}
		...
		if( $tabs = $this->format_attribute( '', 'tabs', $attributes, false, '' ) ){
			...
			for( $cT = 1; $cT <= $tabs; $cT++ ){
				...
				$html .= '<div id="tabs-'.esc_attr( $cT ).'" class="ui-tabs-panel ui-widget-content ui-corner-bottom">';
