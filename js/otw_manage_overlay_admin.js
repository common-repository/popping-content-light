...
function init_manage_page(){
	...
	var s_labels = jQuery( '.otw_overlay_item_filter label' ).on( 'click',   function( event ){
	...
	jQuery( '#full_bar_position, #side_box_position' ).on( 'change',  function(){
	...
	jQuery( '#side_box_position' ).on( 'change',  function(){
	...
	jQuery('#otw-col-left').find('.sitem_toggle, .sitem_header').on( 'click', function() {
		...
		if( jQuery(this).parent().attr( 'data-type' ) ){
			...
			post_params.page = 'otw-pcl-manage';
			...
			var req_url = 'admin-ajax.php?action=otw_pcl_admin_settings';
	...
	jQuery( '.otw_info_box .otw_info_box_controls a' ).on( 'click',  function(){
	...
	jQuery( '.otw_info_box .otw_info_box_content a.otw_scrollto' ).on( 'click',  function(){
